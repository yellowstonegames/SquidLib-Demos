...
//    private WorldMapGenerator.MimicMap world;
    ...
    public void create() {
        ...
        TextCellFactory tcf = DefaultResources.getCrispSlabFamily();
...
//        world = new WorldMapGenerator.MimicMap(seed, WorldMapGenerator.DEFAULT_NOISE, 0.8); // uses a map of Australia for land
        wild = new WildMap(256, 256, 21, new SilkRNG("SquidLib!"), WildMap.makeVegetation(rng, 70, 0.2, FakeLanguageGen.SIMPLISH));
        ...
        final char[] options = "¥¥¥¥♣♣♣*".toCharArray(); // what we can automatically place for vegetation; ignores WildMap defaults
        for (int i = 0; i < wild.contentTypes.size(); i++) {
            viewer.put(wild.contentTypes.get(i), new Basic(options[wild.rng.next(3)], AURORA_AVOCADO.toEditedFloat(wild.rng.nextFloat(0.35f) - 0.25f, wild.rng.nextFloat(0.4f) - 0.2f, wild.rng.nextFloat(0.3f) - 0.45f)));
        }
