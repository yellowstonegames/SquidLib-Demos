 ...
 * passes at least 8TB of PractRand's statistical tests, and does so with no anomalies and no failures (tests are
 * ongoing for the 16TB and 32TB points of the battery). In statistical testing, xoroshiro128+ always fails some binary
 * matrix rank tests, but that uses a pair of 64-bit states, and when the states are reduced to 32-bits, these
 * small-word versions fail other tests as well. Starfish uses a simpler variant on xoroshiro64** that reduces some
 * issues with that generator and trades them for better-understood issues. Starfish does not change xoroshiro's
 * well-tested state transition, but it doesn't base the output on the sum of the two states (like xoroshiro128+),
 * instead using the first state only for output (exactly like xoroshiro64** and similar to xoshiro256**). Any
 * arithmetic it performs is safe for GWT. Starfish makes an extremely small set of changes to xoroshiro, running
 * xoroshiro's state transition as normal, using stateA (or s[0] in the original xoroshiro code) multiplied by 31 as the
 * initial result, then bitwise-rotating that initial result by 28 and adding a constant that is close to 2 to the 32
 * times the golden ratio, specifically {@code 0x9E3779BD}. Although no bits of xoroshiro are truly free of artifacts,
 * some are harder to find issues with
 ...
 * 32-bit states, at 0xFFFFFFFFFFFFFFFF or 2 to the 64 minus 1. This generator is somewhat slower than xoroshiro64+ or
 * Lathe, but has better distribution than either; it can be contrasted with {@link XoshiroAra32RNG}, which is slightly
 * slower on GWT but is 4-dimensionally equidistributed and has a much larger period (and state size).
 * <br>
 ...
 * cause similar binary matrix rank failures.
 * <br>
