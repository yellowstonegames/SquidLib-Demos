 ...
 * this is the fastest generator on GWT I have found that also seems to pass the full 32TB battery of PractRand's
 * statistical tests. In statistical testing, xoroshiro always fails some binary matrix rank tests, but smaller-state
 * versions fail other tests as well. The changes Oriole makes apply only to the output of xoroshiro, not its
 * well-tested state transition for the "xoroshiro state" part of this generator, and these changes eliminate all
 * statistical failures on 32TB of tested data, avoiding the failures the small-state variant of xoroshiro suffers on
 * BCFN, DC6, and FPF. It avoids multiplication, like xoroshiro and much of the xorshift family of generators, and any 
 * arithmetic it performs is safe for GWT. Oriole takes advantage of xoroshiro's issues being mostly confined to its
 * output's lower bits by rotating the output of xoroshiro (the weaker 32-bit-state version) and adding a "Weyl
 * sequence," essentially a large-increment counter that overflows and wraps frequently, to the output. Although the
 * upper bits of xoroshiro are not free of artifacts either, they are harder to find issues with
 ...
 * would help to some extent with at least the binary rank failures. The period is also improved by incorporating the
 * Weyl sequence, up to 0xFFFFFFFFFFFFFFFF00000000 .
 * <br>
