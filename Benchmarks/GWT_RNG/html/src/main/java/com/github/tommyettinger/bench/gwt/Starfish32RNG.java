 ...
 * passes al 32TB of PractRand's statistical tests, and does so with no anomalies and no failures (the best possible
 ...
 * xoroshiro code) multiplied by 31 as theinitial result, then bitwise-rotating that initial result by 28 and adding a
 ...
 * Lathe, but has better distribution than either; it can be contrasted with {@link XoshiroAra32RNG}, which is somewhat
 * slower on GWT but is 4-dimensionally equidistributed and has a much larger period (and state size). XoshiroAra also
 * has not passed Practrand without anomalies; many seeds when given to XoshiroAra will produce a fair amount of
 * anomalies, usually not considered severe on their own, though.
 * <br>
