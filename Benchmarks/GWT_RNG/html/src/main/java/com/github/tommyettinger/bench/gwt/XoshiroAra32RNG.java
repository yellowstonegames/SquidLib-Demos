 ...
 * help make most issues less severe. XoshiroAra32RNG is optimized for GWT, like Lathe32RNG and
 ...
 * <br>
 * This generator seems to be faster than {@link XoshiroStarStar32RNG} while offering the same period and
 ...
 */
public final class XoshiroAra32RNG implements RandomnessSource, Serializable {
    ...
    /**
     * Constructs this XoshiroAra32RNG by dispersing the bits of seed using {@link #setSeed(int)} across the four
     * parts of state this has.
     * @param seed an int that won't be used exactly, but will affect all components of state
     */
    public XoshiroAra32RNG(final int seed) {
        setSeed(seed);
    }
    /**
     * Constructs this XoshiroAra32RNG by dispersing the bits of seed using {@link #setSeed(long)} across the four
     * parts of state this has.
     * @param seed a long that will be split across all components of state
     */
    public XoshiroAra32RNG(final long seed) {
        setSeed(seed);
    }
    /**
     * Constructs this XoshiroAra32RNG by calling {@link #setState(int, int, int, int)} on stateA and stateB as
     ...
     */
    public XoshiroAra32RNG(final int stateA, final int stateB, final int stateC, final int stateD) {
        setState(stateA, stateB, stateC, stateD);
    }

    @Override
    public final int next(int bits) {
        final int result = stateB + 0x41C64E6D;
...
//        return (result ^ result >>> 17) * 3 >>> (32 - bits);
        return (result << 17 | result >>> 15) + 0x9E3779B9 >>> (32 - bits);
    }
...
//        return (result ^ result >>> 17) * 3 | 0;
        return (result << 17 | result >>> 15) + 0x9E3779B9 | 0;
    }

    @Override
    public final long nextLong() {
        int result = stateB + 0x41C64E6D;
        int t = stateB << 9;
        stateC ^= stateA;
        stateD ^= stateB;
        stateB ^= stateC;
...
//        long high = (result ^ result >>> 17) * 3;
        long high = (result << 17 | result >>> 15) + 0x9E3779B9;
        stateD = (stateD << 11 | stateD >>> 21);
        result = stateB + 0x41C64E6D;
        t = stateB << 9;
        stateC ^= stateA;
        stateD ^= stateB;
        stateB ^= stateC;
...
//        return high << 32 ^ ((result ^ result >>> 17) * 3);
        return high << 32 ^ ((result << 17 | result >>> 15) + 0x9E3779B9);
    }
     ...
     */
    @Override
    public XoshiroAra32RNG copy() {
        return new XoshiroAra32RNG(stateA, stateB, stateC, stateD);
    }
    ...
    /**
     * Sets the current internal state of this XoshiroAra32RNG with four ints, where each can be any int unless
    ...
    public String toString() {
        return "XoshiroAra32RNG with stateA 0x" + StringKit.hex(stateA) + ", stateB 0x" + StringKit.hex(stateB)
    ...
    public boolean equals(Object o) {
        ...
        XoshiroAra32RNG xoshiroAra32RNG = (XoshiroAra32RNG) o;

        return stateA == xoshiroAra32RNG.stateA && stateB == xoshiroAra32RNG.stateB &&
                stateC == xoshiroAra32RNG.stateC && stateD == xoshiroAra32RNG.stateD;
    }
