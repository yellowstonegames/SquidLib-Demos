 ...
 * and LightRNG takes roughly 2480 microseconds." The total period of Zug32RNG is 0xFFFFFFFF00000000
 * (18446744069414584320). Quality is excellent here, and this passes PractRand without failures and with 2 minor
 * anomalies (classified as "unusual") on the full 32TB of testing.
 * <br>
 * For the specific structure of Zug32RNG, it is a combination of a Marsaglia-style XorShift generator with a "XLCG"
 * generator (like a linear congruential generator but using XOR instead of addition; this also uses a smaller-magnitude
 * multiplier than normal to avoid GWT precision loss by using more than 53 bits). It has 2 ints for state, stateA
 ...
 * quality; the XLCG's result (not the actual stateB) is xorshifted right twice, as in {@code tempB ^= tempB >>> 10},
 * before adding the XorShift result of stateA and returning that sum. The XorShift that stateA uses is a normal kind
 * that involves two rightward shifts and one leftward shift, but because stateB is updated by an XLCG (which has a
...
public final class Zug32RNG implements StatefulRandomness, Serializable {
    ...
    public final long nextLong() {
        int z = (b ^ 0xC74EAD55) * 0xA5CB3 | 0, y = (z ^ 0xC74EAD55) * 0xA5CB3 | 0;
        a ^= a >>> 14;
        z ^= z >>> 10;
        a ^= a >>> 15;
        z = (z ^ z >>> 20) + (a ^= a << 13) | 0;
    ...
    {
        return ((bound * ((((b ^= b >>> 10) ^ b >>> 20) + a) & 0x7FFF)) >> 15);
    }
    /**
