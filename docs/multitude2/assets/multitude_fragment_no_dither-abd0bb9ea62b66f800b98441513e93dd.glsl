...
vec3 bas(vec3 x)
{
    return cross(sway(v_section, x), swayTrig(v_section - 0.511, x.yzx * 0.5));
//    return sway(v_section, x) * 0.375 + sway(0.618 - v_section, x * 0.5) * 0.3125 + swayTrig(v_section - 0.511, x * 0.75);
}

// this is different from other swayRandomized in Northern demos because it uses Quilez basic noise and trig
