 ...
 * {@link #expand()}), {@link #retract()}), {@link #fringe()}), {@link #surface()}, and {@link #flood(GreasedRegion2)},
 * and slightly faster on others, like {@link #and(GreasedRegion2)} (called intersectPacked() in CoordPacker) and
 * {@link #or(GreasedRegion2)} (called unionPacked() in CoordPacker).
 * <br>
 * Each GreasedRegion2 is mutable, and instance methods typically modify that instance and return it for chaining. There
 * are exceptions, usually where multiple GreasedRegion2 values are returned and the instance is not modified.
 * <br>
 * Typical usage involves constructing a GreasedRegion2 from some input data, like a char[][] for a map or a double[][]
 ...
 * are either 4-way (Manhattan distance) or not affected by distance measurement. Once you have a GreasedRegion2, you may
 * want to:
 * <ul>
 ...
 *     <li>mutate the current GreasedRegion2 to keep random points from it with random sampling (use {@link #randomRegion(IRNG, int)}),</li>
 ...
 *     <li>do what any of the above "separated" methods can do, but mutate the current GreasedRegion2 (use
 ...
 * </ul>
 * <br>
 * You may also want to produce some 2D data from one or more GreasedRegions, as with {@link #sum(GreasedRegion2...)} or
 * {@link #toChars()}. The most effective techniques regarding GreasedRegion2 involve multiple methods, like getting a
 * few random points from an existing GreasedRegion2 representing floor tiles in a dungeon with
 * {@link #randomRegion(IRNG, int)}, then finding a random expansion of those initial points with
 * {@link #spill(GreasedRegion2, int, IRNG)}, giving the original GreasedRegion2 of floor tiles as the first argument.
 ...
 * <br>
 * For efficiency, you can place one GreasedRegion2 into another (typically a temporary value that is no longer needed
 * and can be recycled) using {@link #remake(GreasedRegion2)}, or give the information that would normally be used to
 * construct a fresh GreasedRegion2 to an existing one of the same dimensions with {@link #refill(boolean[][])} or any
 ...
 */
@Beta
public class GreasedRegion2 extends Zone.Skeleton implements Collection<Coord>, Serializable, MutableZone {
    ...
    /**
     * Constructs a GreasedRegion2 with the given rectangular boolean array, with width of bits.length and height of
     ...
     */
    public GreasedRegion2(final boolean[][] bits)
    {
        width = bits.length;
    ...
    /**
     * Reassigns this GreasedRegion2 with the given rectangular boolean array, reusing the current data storage (without
     ...
     */
    public GreasedRegion2 refill(final boolean[][] map) {
    ...
    /**
     * Constructs a GreasedRegion2 with the given rectangular char array, with width of map.length and height of
     ...
     */
    public GreasedRegion2(final char[][] map, final char yes)
    {
        width = map.length;
        height = map[0].length;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Reassigns this GreasedRegion2 with the given rectangular char array, reusing the current data storage (without
     * extra allocations) if this.width == map.length and this.height == map[0].length. The current values stored in
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final char[][] map, final char yes) {
        if (map != null && map.length > 0 && width == map.length && height == map[0].length) {
            Arrays.fill(data, 0L);
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
     ...
     * @param yes the char to consider "on" in the GreasedRegion2
     */
    public GreasedRegion2(final String[] map, final char yes)
    {
        height = map.length;
        width = map[0].length();
        ySections = (height + 63) >> 6;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final String[] map, final char yes) {
    ...
    /**
     * Constructs a GreasedRegion2 with the given rectangular int array, with width of map.length and height of
     * map[0].length, any value that equals yes is considered "on", and any other value considered "off."
     ...
     */
    public GreasedRegion2(final int[][] map, final int yes)
    {
        width = map.length;
        height = map[0].length;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Reassigns this GreasedRegion2 with the given rectangular int array, reusing the current data storage (without
     * extra allocations) if this.width == map.length and this.height == map[0].length. The current values stored in
     * this are always cleared, then any value that equals yes is considered "on", and any other value considered "off."
     * @param map a rectangular 2D int array where an int == yes is on and everything else is off
     * @param yes which int to encode as "on"
     * @return this for chaining
     */
    public GreasedRegion2 refill(final int[][] map, final int yes) {
        if (map != null && map.length > 0 && width == map.length && height == map[0].length) {
            Arrays.fill(data, 0L);
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
    ...
    /**
     * Constructs this GreasedRegion2 using an int[][], treating cells as on if they are greater than or equal to lower
     ...
     */
    public GreasedRegion2(final int[][] map, final int lower, final int upper)
    {
        width = map.length;
        height = map[0].length;
        ySections = (height + 63) >> 6;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final int[][] map, final int lower, final int upper) {
        if (map != null && map.length > 0 && width == map.length && height == map[0].length) {
            Arrays.fill(data, 0L);
            int[] column;
            for (int x = 0; x < width; x++) {
    ...
    /**
     * Constructs this GreasedRegion2 using a short[][], treating cells as on if they are greater than or equal to lower
     * and less than upper, or off otherwise.
     * @param map a short[][] that should have some shorts between lower and upper
     * @param lower lower bound, inclusive; all on cells will have values in map that are at least equal to lower
     * @param upper upper bound, exclusive; all on cells will have values in map that are less than upper
     */
    public GreasedRegion2(final short[][] map, final int lower, final int upper)
    {
        width = map.length;
        height = map[0].length;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Reassigns this GreasedRegion2 with the given rectangular short array, reusing the current data storage (without
     * extra allocations) if this.width == map.length and this.height == map[0].length. The current values stored in
     * this are always cleared, then cells are treated as on if they are greater than or equal to lower and less than
     * upper, or off otherwise.
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final short[][] map, final int lower, final int upper) {
        if (map != null && map.length > 0 && width == map.length && height == map[0].length) {
            Arrays.fill(data, 0L);
            short[] column;
            for (int x = 0; x < width; x++) {
    ...
    /**
     * Constructs this GreasedRegion2 using a double[][] (typically one generated by
     ...
     */
    public GreasedRegion2(final double[][] map, final double upperBound)
    {
        width = map.length;
        height = map[0].length;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Reassigns this GreasedRegion2 with the given rectangular double array, reusing the current data storage (without
     * extra allocations) if this.width == map.length and this.height == map[0].length. The current values stored in
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final double[][] map, final double upperBound) {
        if (map != null && map.length > 0 && width == map.length && height == map[0].length) {
            Arrays.fill(data, 0L);
            for (int x = 0; x < width; x++) {
                for (int y = 0; y < height; y++) {
     ...
     */
    public GreasedRegion2(final double[][] map, final double lowerBound, final double upperBound)
    {
        width = map.length;
        height = map[0].length;
        ySections = (height + 63) >> 6;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final double[][] map, final double lower, final double upper) {
        if (map != null && map.length > 0 && width == map.length && height == map[0].length) {
            Arrays.fill(data, 0L);
            double[] column;
            for (int x = 0; x < width; x++) {
    ...
    /**
     * Constructs this GreasedRegion2 using a double[][] (this was made for use inside
     ...
     */
    public GreasedRegion2(final double[][] map, final double lowerBound, final double upperBound, int scale)
    {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final double[][] map, final double lowerBound, final double upperBound, int scale) {
        scale = Math.min(63, Math.max(1, scale));
    ...
    /**
     * Constructs a GreasedRegion2 with the given 1D boolean array, with the given width and height, where an [x][y]
     ...
     * @param width the width of the desired GreasedRegion2; width * height should equal bits.length
     * @param height the height of the desired GreasedRegion2; width * height should equal bits.length
     */
    public GreasedRegion2(final boolean[] bits, final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Reassigns this GreasedRegion2 with the given 1D boolean array, reusing the current data storage (without
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final boolean[] bits, final int width, final int height) {
    ...
    /**
     * Constructor for an empty GreasedRegion2 of the given width and height.
     * GreasedRegions are mutable, so you can add to this with insert() or insertSeveral(), among others.
     * @param width the maximum width for the GreasedRegion2
     * @param height the maximum height for the GreasedRegion2
     */
    public GreasedRegion2(final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * If this GreasedRegion2 has the same width and height passed as parameters, this acts the same as {@link #empty()},
     * makes no allocations, and returns this GreasedRegion2 with its contents all "off"; otherwise, this does allocate
     * a differently-sized amount of internal data to match the new width and height, sets the fields to all match the
     * new width and height, and returns this GreasedRegion2 with its new width and height, with all contents "off". This
     * is meant for cases where a GreasedRegion2 may be reused effectively, but its size may not always be the same.
     * @param width the width to potentially resize this GreasedRegion2 to
     * @param height the height to potentially resize this GreasedRegion2 to
     * @return this GreasedRegion2, always with all contents "off", and with the height and width set.
     */
    public GreasedRegion2 resizeAndEmpty(final int width, final int height) {
    ...
    /**
     * Constructor for a GreasedRegion2 that contains a single "on" cell, and has the given width and height.
     ...
     * @param single the one (x,y) point to store as "on" in this GreasedRegion2
     * @param width the maximum width for the GreasedRegion2
     * @param height the maximum height for the GreasedRegion2
     */
    public GreasedRegion2(final Coord single, final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Constructor for a GreasedRegion2 that can have several "on" cells specified, and has the given width and height.
     ...
     * @param width the maximum width for the GreasedRegion2
     * @param height the maximum height for the GreasedRegion2
     * @param points an array or vararg of Coord to store as "on" in this GreasedRegion2
     */
    public GreasedRegion2(final int width, final  int height, final Coord... points)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
     ...
     * @param points an array or vararg of Coord to store as "on" in this GreasedRegion2
     */
    public GreasedRegion2(final int width, final int height, final Iterable<Coord> points)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Constructor for a random GreasedRegion2 of the given width and height, typically assigning approximately half of
     ...
     * @param height the maximum height for the GreasedRegion2
     */
    public GreasedRegion2(final RandomnessSource random, final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Reassigns this GreasedRegion2 by filling it with random values from random, reusing the current data storage
     ...
     * @param random a RandomnessSource that should have a good nextLong() method; LinnormRNG is excellent but Lathe32RNG is faster on GWT (only there) 
     * @param width the width of the desired GreasedRegion2
     * @param height the height of the desired GreasedRegion2
     * @return this for chaining
     */
    public GreasedRegion2 refill(final RandomnessSource random, final int width, final int height) {
        if (random != null){
     ...
     * Constructor for a random GreasedRegion2 of the given width and height, typically assigning approximately half of
     * the cells in this to "on" and the rest to off. A RandomnessSource can be slightly more efficient than an RNG when
     * you're making a lot of calls on it, so you may prefer {@link #GreasedRegion2(RandomnessSource, int, int)}.
     * @param random an IRNG, such as an RNG, that this will use to generate its contents
     * @param width the maximum width for the GreasedRegion2
     * @param height the maximum height for the GreasedRegion2
     */
    public GreasedRegion2(final IRNG random, final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 refill(final IRNG random, final int width, final int height) {
        if (random != null){
            if(this.width == width && this.height == height) {
                for (int i = 0; i < width * ySections; i++) {
                    data[i] = random.nextLong();
    ...
    public GreasedRegion2(final RandomnessSource random, final double fraction, final int width, final int height)
    {
        this.width = width;
        this.height = height;
    ...
    public GreasedRegion2 refill(final RandomnessSource random, final double fraction, final int width, final int height) {
        if (random != null){
            int bitCount = (int) (fraction * 64);
            if(this.width == width && this.height == height) {
                for (int i = 0; i < width * ySections; i++) {
    ...
    /**
     * Copy constructor that takes another GreasedRegion2 and copies all of its data into this new one.
     * If you find yourself frequently using this constructor and assigning it to the same variable, consider using the
     * {@link #remake(GreasedRegion2)} method on the variable instead, which will, if it has the same width and height
     * as the other GreasedRegion2, avoid creating garbage and quickly fill the variable with the other's contents.
     * @see #copy() for a convenience method that just uses this constructor
     * @param other another GreasedRegion2 that will be copied into this new GreasedRegion2
     */
    public GreasedRegion2(final GreasedRegion2 other)
    {
        width = other.width;
        height = other.height;
     ...
     * GreasedRegion2 will use, and does not perform any validation steps to ensure that cells that would be "on" but are
     * outside the actual height of the GreasedRegion2 are actually removed (this only matters if height is not a
     * multiple of 64).
     * @param data2 a long array that is typically from another GreasedRegion2, and would be hard to make otherwise
     * @param width the width of the GreasedRegion2 to construct
     * @param height the height of the GreasedRegion2 to construct
     */
    public GreasedRegion2(final long[] data2, final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Primarily for internal use, this constructor copies data2 into the internal long array the new GreasedRegion2 will
     * use, but treats data2 as having the dimensions [dataWidth][dataHeight], and uses the potentially-different
     * dimensions [width][height] for the constructed GreasedRegion2. This will truncate data2 on width, height, or both
     ...
     * @param height the height of the GreasedRegion2 to construct
     */
    public GreasedRegion2(final long[] data2, final int dataWidth, final int dataHeight, final int width, final int height)
    {
        this.width = width;
        this.height = height;
        ySections = (height + 63) >> 6;
    ...
    /**
     * Primarily for internal use, this method copies data2 into the internal long array the new GreasedRegion2 will
     * use, but treats data2 as having the dimensions [dataWidth][dataHeight], and uses the potentially-different
     * dimensions [width][height] for this GreasedRegion2, potentially re-allocating the internal data this uses if width
     ...
     * @param dataWidth the width to interpret data2 as having
     * @param dataHeight the height to interpret data2 as having
     * @param width the width to set this GreasedRegion2 to have
     * @param height the height to set this GreasedRegion2 to have
     */
    public GreasedRegion2 refill(final long[] data2, final int dataWidth, final int dataHeight, final int width, final int height)
    {
        if(width != this.width || height != this.height) {
            this.width = width;
            this.height = height;
    ...
    /**
     * A useful method for efficiency, remake() reassigns this GreasedRegion2 to have its contents replaced by other. If
     * other and this GreasedRegion2 have identical width and height, this is very efficient and performs no additional
     * allocations, simply replacing the cell data in this with the cell data from other. If width and height are not
     * both equal between this and other, this does allocate a new data array, but still reassigns this GreasedRegion2
     ...
     * maintaining good performance and memory usage. You often can recycle a no-longer-used GreasedRegion2 by assigning
     * a GreasedRegion2 you want to keep to it with remake(), then mutating either the remade value or the one that was
     * just filled into this but keeping one version around for later usage.
     * @param other another GreasedRegion2 to replace the data in this GreasedRegion2 with
     * @return this for chaining
     */
    public GreasedRegion2 remake(GreasedRegion2 other) {
    ...
    /**
     * Changes the width and/or height of this GreasedRegion2, enlarging or shrinking starting at the edges where
     ...
     * use {@code insert(int, int, GreasedRegion2)} if you want to place one GreasedRegion2 inside another one,
     ...
     * @return this for chaining
     */
    public GreasedRegion2 alterBounds(int widthChange, int heightChange)
    {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 set(boolean value, int x, int y)
    {
        if(x < width && y < height && x >= 0 && y >= 0) {
            if(value)
                data[x * ySections + (y >> 6)] |= 1L << (y & 63);
     ...
     * @return this for chaining
     */
    public GreasedRegion2 set(boolean value, Coord point)
    {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 insert(int x, int y)
    {
        if(x < width && y < height && x >= 0 && y >= 0)
        {
            data[x * ySections + (y >> 6)] |= 1L << (y & 63);
     ...
     * @param tight a cell tightly encoded for this GreasedRegion2's width and height
     * @return this for chaining
     */
    public GreasedRegion2 insert(int tight)
    {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 insert(Coord point)
    {
    ...
    /**
     * Takes another GreasedRegion2, called other, with potentially different size and inserts its "on" cells into thi
     * GreasedRegion2 at the given x,y offset, allowing negative x and/or y to put only part of other in this.
     * <br>
     ...
     * @param other the other GreasedRegion2 to insert
     * @return this for chaining
     */
    public GreasedRegion2 insert(int x, int y, GreasedRegion2 other)
    {
    ...
    public GreasedRegion2 insertSeveral(Coord... points)
    {
        for (int i = 0, x, y; i < points.length; i++) {
            x = points[i].x;
            y = points[i].y;
    ...
    public GreasedRegion2 insertSeveral(final int[] points)
    {
    ...
    public GreasedRegion2 insertSeveral(Iterable<Coord> points)
    {
        int x, y;
        for (Coord pt : points) {
            x = pt.x;
    ...
    public GreasedRegion2 insertRectangle(int startX, int startY, int rectangleWidth, int rectangleHeight)
    {
    ...
    public GreasedRegion2 insertCircle(Coord center, int radius)
    {
        float high, changedX;
        int rndX, rndY;
    ...
    public GreasedRegion2 remove(int x, int y)
    {
        if(x < width && y < height && x >= 0 && y >= 0)
        {
            data[x * ySections + (y >> 6)] &= ~(1L << (y & 63));
    ...
    public GreasedRegion2 remove(Coord point)
    {
        return remove(point.x, point.y);
    }
    /**
     * Takes another GreasedRegion2, called other, with potentially different size and removes its "on" cells from this
     * GreasedRegion2 at the given x,y offset, allowing negative x and/or y to remove only part of other in this.
     * <br>
     * This is a rather complex method internally, but should be about as efficient as a general remove-region method
     * can be. The code is identical to {@link #insert(int, int, GreasedRegion2)} except that where insert only adds
     * cells, this only removes cells. Essentially, insert() is to {@link #or(GreasedRegion2)} as remove() is to
     * {@link #andNot(GreasedRegion2)}.
     ...
     * @param other the other GreasedRegion2 to remove
     * @return this for chaining
     */
    public GreasedRegion2 remove(int x, int y, GreasedRegion2 other)
    {
        if(other == null || other.ySections <= 0 || other.width <= 0)
            return this;
    ...
    public GreasedRegion2 removeSeveral(Coord... points)
    {
        for (int i = 0, x, y; i < points.length; i++) {
            x = points[i].x;
            y = points[i].y;
    ...
    public GreasedRegion2 removeSeveral(Iterable<Coord> points)
    {
        int x, y;
        for (Coord pt : points) {
            x = pt.x;
    ...
    public GreasedRegion2 removeRectangle(int startX, int startY, int rectangleWidth, int rectangleHeight)
    {
        if(startX < 0)
        {
    ...
    public GreasedRegion2 removeCircle(Coord center, int radius)
    {
        float high, changedX;
        int rndX, rndY;
        for (int dx = -radius; dx <= radius; ++dx) {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 fill(boolean contents)
    {
        if(contents)
        {
            if(ySections > 0)
    ...
    /**
     * Turns all cells that are adjacent to the boundaries of the GreasedRegion2 to "off".
     * @return this for chaining
     */
    public GreasedRegion2 removeEdges()
    {
        if(ySections > 0) {
    ...
    /**
     * Simple method that returns a newly-allocated copy of this GreasedRegion2; modifications to one won't change the
     * other, and this method returns the copy while leaving the original unchanged.
     * @return a copy of this GreasedRegion2; the copy can be changed without altering the original
     */
    public GreasedRegion2 copy()
    {
        return new GreasedRegion2(this);
    }

    /**
     * Returns this GreasedRegion2's data as a 2D boolean array, [width][height] in size, with on treated as true and off
     * treated as false.
     * @return a 2D boolean array that represents this GreasedRegion2's data
     */
    ...
    /**
     * Fills this GreasedRegion2's data into the given 2D char array, modifying it and returning it, with "on" cells
     ...
     * @return a 2D char array that represents this GreasedRegion2's data
     */
     ...
     * @param chars a 2D char array that will be modified; must not be null, nor can it contain null elements
     * @param on the char to use for "on" cells
     * @return a 2D char array that represents the "on" cells in this GreasedRegion2's data written over chars
     */
    ...
    /**
     * Returns this GreasedRegion2's data as a 2D char array,  [width][height] in size, with "on" cells filled with the
    ...
    /**
     * Returns this GreasedRegion2's data as a 2D char array,  [width][height] in size, with "on" cells filled with '.'
     * and "off" cells with '#'.
     * @return a 2D char array that represents this GreasedRegion2's data
     */
    ...
    /**
     * Returns this GreasedRegion2's data as a StringBuilder, with each row made of the parameter on for "on" cells and
     * the parameter off for "off" cells, separated by newlines, with no trailing newline at the end.
     * @param on the char to use for "on" cells
     * @param off the char to use for "off" cells
     * @return a StringBuilder that stores each row of this GreasedRegion2 as chars, with rows separated by newlines.
     */
     ...
     * @return a String representation of this GreasedRegion2 using '.' for on, '#' for off, and newlines between rows
     */
    @Override
    ...
    /**
     * Returns a copy of map where if a cell is "on" in this GreasedRegion2, this keeps the value in map intact,
     * and where a cell is "off", it instead writes the char filler.
     * @param map a 2D char array that will not be modified
     * @param filler the char to use where this GreasedRegion2 stores an "off" cell
     ...
     * @param filler the short to use where this GreasedRegion2 stores an "off" cell
     * @return a masked copy of map
     */
    ...
    /**
     * Returns a copy of map where if a cell is "off" in this GreasedRegion2, this keeps the value in map intact,
     * and where a cell is "on", it instead writes the char toWrite.
     * @param map a 2D char array that will not be modified
     * @param toWrite the char to use where this GreasedRegion2 stores an "on" cell
     * @return a masked copy of map
     */
    ...
    /**
     * "Inverse mask for ints;" returns a copy of map where if a cell is "off" in this GreasedRegion2, this keeps
     ...
     * @param toWrite the int to use where this GreasedRegion2 stores an "on" cell
    ...
    /**
     * "Inverse mask for doubles;" returns a copy of map where if a cell is "off" in this GreasedRegion2, this keeps
     ...
     * @param toWrite the double to use where this GreasedRegion2 stores an "on" cell
     * @return an altered copy of map
     */
    ...
    /**
     * Union of two GreasedRegions, assigning the result into this GreasedRegion2. Any cell that is "on" in either
     * GreasedRegion2 will be made "on" in this GreasedRegion2.
     * @param other another GreasedRegion2 that will not be modified
     * @return this, after modification, for chaining
     */
    public GreasedRegion2 or(GreasedRegion2 other)
    {
    ...
    /**
     * Intersection of two GreasedRegions, assigning the result into this GreasedRegion2. Any cell that is "on" in both
     * GreasedRegions will be kept "on" in this GreasedRegion2, but all other cells will be made "off."
     * @param other another GreasedRegion2 that will not be modified
     * @return this, after modification, for chaining
     */
    public GreasedRegion2 and(GreasedRegion2 other)
    {
        for (int x = 0; x < width && x < other.width; x++) {
            for (int y = 0; y < ySections && y < other.ySections; y++) {
                data[x * ySections + y] &= other.data[x * ySections + y];
    ...
    /**
     * Difference of two GreasedRegions, assigning the result into this GreasedRegion2. Any cell that is "on" in this
     * GreasedRegion2 and "off" in other will be kept "on" in this GreasedRegion2, but all other cells will be made "off."
     * @param other another GreasedRegion2 that will not be modified
     * @return this, after modification, for chaining
     * @see #notAnd(GreasedRegion2) notAnd is a very similar method that acts sort-of in reverse of this method
     */
    public GreasedRegion2 andNot(GreasedRegion2 other)
    {
        for (int x = 0; x < width && x < other.width; x++) {
            for (int y = 0; y < ySections && y < other.ySections; y++) {
                data[x * ySections + y] &= ~other.data[x * ySections + y];
    ...
    /**
     * Like andNot, but subtracts this GreasedRegion2 from other and stores the result in this GreasedRegion2, without
     * mutating other.
     * @param other another GreasedRegion2 that will not be modified
     * @return this, after modification, for chaining
     * @see #andNot(GreasedRegion2) andNot is a very similar method that acts sort-of in reverse of this method
     */
    public GreasedRegion2 notAnd(GreasedRegion2 other)
    {
        for (int x = 0; x < width && x < other.width; x++) {
            for (int y = 0; y < ySections && y < other.ySections; y++) {
     ...
     * result into this GreasedRegion2. Any cell that is "on" in this and "off" in other, or "off" in this and "on" in
     ...
     * @return this, after modification, for chaining
     */
    public GreasedRegion2 xor(GreasedRegion2 other)
    {
        for (int x = 0; x < width && x < other.width; x++) {
            for (int y = 0; y < ySections && y < other.ySections; y++) {
                data[x * ySections + y] ^= other.data[x * ySections + y];
    ...
    /**
     * Negates this GreasedRegion2, turning "on" to "off" and "off" to "on."
     * @return this, after modification, for chaining
     */
    public GreasedRegion2 not()
    {
    ...
    /**
     * Moves the "on" cells in this GreasedRegion2 to the given x and y offset, removing cells that move out of bounds.
     * @param x the x offset to translate by; can be negative
     * @param y the y offset to translate by; can be negative
     * @return this for chaining
     */
    public GreasedRegion2 translate(int x, int y)
    {
    ...
    /**
     * Adds to this GreasedRegion2 with a moved set of its own "on" cells, moved to the given x and y offset.
     * Ignores cells that would be added out of bounds. Keeps all cells that are currently "on" unchanged.
     * @param x the x offset to translate by; can be negative
     * @param y the y offset to translate by; can be negative
     * @return this for chaining
     */
    public GreasedRegion2 insertTranslation(int x, int y)
    {
        if(width < 1 || ySections <= 0 || (x == 0 && y == 0))
            return this;
        int start = Math.max(0, x), len = Math.min(width, width + x) - start,
     ...
     * become 0,0 in the resulting GreasedRegion2 (which is this, assigning to itself).
     ...
     * @return this for chaining
     */
    public GreasedRegion2 zoom(int x, int y)
    {
    ...
    /**
     * Takes the pairs of "on" cells in this GreasedRegion2 that are separated by exactly one cell in an orthogonal line,
    ...
    /**
     * Takes the pairs of "on" cells in this GreasedRegion2 that are separated by exactly one cell in an orthogonal or
     ...
     * @return this for chaining
     */
    public GreasedRegion2 connect8way()
    {
        if(width < 2 || ySections == 0)
            return this;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 connectLines()
    {
        if(width < 2 || ySections == 0)
            return this;
    ...
    /**
     * Like {@link #retract()}, this reduces the width of thick areas of this GreasedRegion2, but thin() will not remove
     ...
     * @return this for chaining
     */
    public GreasedRegion2 thin()
    {
        if(width <= 2 || ySections <= 0)
            return this;
        GreasedRegion2 c1 = new GreasedRegion2(this).retract(),
                c2 = new GreasedRegion2(c1).expand().xor(this).expand().and(this);
        remake(c1).or(c2);
        /*
        System.out.println("\n\nc1:\n" + c1.toString() + "\n");
        System.out.println("\n\nc2:\n" + c2.toString() + "\n");
     ...
     * {@code expand8way().retract().thinFully()} to help change a possibly-strange appearance when the GreasedRegion2
     ...
     * @return this for chaining
     */
    public GreasedRegion2 thinFully()
    {
    ...
    /**
     * Like {@link #retract8way()}, this reduces the width of thick areas of this GreasedRegion2, but thin8way() will not
     ...
     * @return this for chaining
     */
    public GreasedRegion2 thin8way()
    {
        if(width <= 2 || ySections <= 0)
            return this;
        GreasedRegion2 c1 = new GreasedRegion2(this).retract8way(),
                c2 = new GreasedRegion2(c1).expand8way().xor(this).expand8way().and(this);
        remake(c1).or(c2);
        return this;
    }
     ...
     * {@code expand8way().retract().thinFully8way()} to help change a strange appearance when the GreasedRegion2 this is
     ...
     * @return this for chaining
     */
    public GreasedRegion2 thinFully8way()
    {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 disperse()
    {
        if(width < 1 || ySections <= 0)
            return this;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 disperse8way()
    {
        if(width < 1 || ySections <= 0)
            return this;
        int len = data.length;
     ...
     * @return this for chaining
     */
    public GreasedRegion2 disperseRandom(RandomnessSource random)
    {
        if(width < 1 || ySections <= 0)
            return this;
        int len = data.length;
     ...
     * <br>
     * Not exactly general-use; meant for generating data for GreasedRegion2.
    ...
    /**
     * Takes the "on" cells in this GreasedRegion2 and expands them by one cell in the 4 orthogonal directions, making
    ...
    /**
     * Takes the "on" cells in this GreasedRegion2 and expands them by amount cells in the 4 orthogonal directions,
     ...
     */
    @Override
    public GreasedRegion2 expand(int amount)
    {
    ...
    /**
     * Takes the "on" cells in this GreasedRegion2 and produces amount GreasedRegions, each one expanded by 1 cell in
     * the 4 orthogonal directions relative to the previous GreasedRegion2, making each "on" cell take up a plus-shaped
     * area that may overlap with other "on" cells (which is just a normal "on" cell then). This returns an array of
     * GreasedRegions with progressively greater expansions, and does not modify this GreasedRegion2.
     * <br>
     * This method is very efficient due to how the class is implemented, and the various spatial increase/decrease
     * methods (including {@link #expand()}, {@link #retract()}, {@link #fringe()}, and {@link #surface()}) all perform
     * very well by operating in bulk on up to 64 cells at a time.
     ...
     */
    public GreasedRegion2[] expandSeries(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
        for (int i = 0; i < amount; i++) {
            regions[i] = new GreasedRegion2(temp.expand());
        }
        return regions;
    }

    public ArrayList<GreasedRegion2> expandSeriesToLimit()
    {
        ArrayList<GreasedRegion2> regions = new ArrayList<>();
        GreasedRegion2 temp = new GreasedRegion2(this);
        while (temp.size() != temp.expand().size()) {
            regions.add(new GreasedRegion2(temp));
        }
        return regions;
    }
    /**
     * Takes the "on" cells in this GreasedRegion2 and expands them by one cell in the 4 orthogonal directions, producing
     ...
     * surface finds cells inside the current GreasedRegion2, while fringe finds cells outside it.
     * <br>
     * This method is very efficient due to how the class is implemented, and the various spatial increase/decrease
     * methods (including {@link #expand()}, {@link #retract()}, {@link #fringe()}, and {@link #surface()}) all perform
     * very well by operating in bulk on up to 64 cells at a time. The surface and fringe methods do allocate one
     * temporary GreasedRegion2 to store the original before modification, but the others generally don't.
     * @return this for chaining
     */
    public GreasedRegion2 fringe()
    {
        GreasedRegion2 cpy = new GreasedRegion2(this);
        expand();
        return andNot(cpy);
    }
    /**
     ...
     * to {@link #surface()}, but surface finds cells inside the current GreasedRegion2, while fringe finds cells outside
     * it.
     * <br>
     * This method is very efficient due to how the class is implemented, and the various spatial increase/decrease
     * methods (including {@link #expand()}, {@link #retract()}, {@link #fringe()}, and {@link #surface()}) all perform
    ...
    public GreasedRegion2 fringe(int amount)
    {
        GreasedRegion2 cpy = new GreasedRegion2(this);
        expand(amount);
        return andNot(cpy);
    ...
    /**
     * Takes the "on" cells in this GreasedRegion2 and produces amount GreasedRegions, each one expanded by 1 cell in
     * the 4 orthogonal directions relative to the previous GreasedRegion2, making each "on" cell take up a diamond-
     * shaped area. After producing the expansions, this removes the previous GreasedRegion2 from the next GreasedRegion2
     ...
     * without the cells of this GreasedRegion2, and does not modify this GreasedRegion2.
     * <br>
     * This method is very efficient due to how the class is implemented, and the various spatial increase/decrease
     * methods (including {@link #expand()}, {@link #retract()}, {@link #fringe()}, and {@link #surface()}) all perform
     * very well by operating in bulk on up to 64 cells at a time.
     ...
     */
    public GreasedRegion2[] fringeSeries(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
        regions[0] = new GreasedRegion2(temp);
    ...
    public ArrayList<GreasedRegion2> fringeSeriesToLimit()
    {
        ArrayList<GreasedRegion2> regions = expandSeriesToLimit();
    ...
    /**
     * Takes the "on" cells in this GreasedRegion2 and retracts them by one cell in the 4 orthogonal directions,
    ...
    /**
     * Takes the "on" cells in this GreasedRegion2 and retracts them by one cell in the 4 orthogonal directions, doing
     ...
     * @return this for chaining
     */
    public GreasedRegion2 retract(int amount)
    {
        for (int i = 0; i < amount; i++) {
            retract();
        }
    ...
    public GreasedRegion2[] retractSeries(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
        for (int i = 0; i < amount; i++) {
            regions[i] = new GreasedRegion2(temp.retract());
        }
        return regions;
    }

    public ArrayList<GreasedRegion2> retractSeriesToLimit()
    {
        ArrayList<GreasedRegion2> regions = new ArrayList<>();
        GreasedRegion2 temp = new GreasedRegion2(this);
    ...
    public GreasedRegion2 surface()
    {
        GreasedRegion2 cpy = new GreasedRegion2(this).retract();
        return xor(cpy);
    }
    public GreasedRegion2 surface(int amount)
    {
        GreasedRegion2 cpy = new GreasedRegion2(this).retract(amount);
        return xor(cpy);
    }

    public GreasedRegion2[] surfaceSeries(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
    ...
    public ArrayList<GreasedRegion2> surfaceSeriesToLimit()
    {
        ArrayList<GreasedRegion2> regions = retractSeriesToLimit();
        if(regions.isEmpty())
            return regions;
    ...
    public GreasedRegion2 expand8way()
    {
        if(width < 2 || ySections <= 0)
            return this;
    ...
    @Override
    public GreasedRegion2 expand8way(int amount)
    {
        for (int i = 0; i < amount; i++) {
            expand8way();
        }
    ...
    public GreasedRegion2[] expandSeries8way(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
        for (int i = 0; i < amount; i++) {
            regions[i] = new GreasedRegion2(temp.expand8way());
        }
        return regions;
    }
    public ArrayList<GreasedRegion2> expandSeriesToLimit8way()
    {
        ArrayList<GreasedRegion2> regions = new ArrayList<>();
        GreasedRegion2 temp = new GreasedRegion2(this);
    ...
    public GreasedRegion2 fringe8way()
    {
        GreasedRegion2 cpy = new GreasedRegion2(this);
        expand8way();
        return andNot(cpy);
    }
    public GreasedRegion2 fringe8way(int amount)
    {
        GreasedRegion2 cpy = new GreasedRegion2(this);
        expand8way(amount);
        return andNot(cpy);
    ...
    public GreasedRegion2[] fringeSeries8way(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
    ...
    public ArrayList<GreasedRegion2> fringeSeriesToLimit8way()
    {
        ArrayList<GreasedRegion2> regions = expandSeriesToLimit8way();
        for (int i = regions.size() - 1; i > 0; i--) {
            regions.get(i).xor(regions.get(i-1));
        }
        regions.get(0).xor(this);
    ...
    public GreasedRegion2 retract8way()
    {
        if(width <= 2 || ySections <= 0)
            return this;
    ...
    public GreasedRegion2 retract8way(int amount)
    {
        for (int i = 0; i < amount; i++) {
            retract8way();
        }
    ...
    public GreasedRegion2[] retractSeries8way(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
        for (int i = 0; i < amount; i++) {
            regions[i] = new GreasedRegion2(temp.retract8way());
        }
        return regions;
    }

    public ArrayList<GreasedRegion2> retractSeriesToLimit8way()
    {
        ArrayList<GreasedRegion2> regions = new ArrayList<>();
        GreasedRegion2 temp = new GreasedRegion2(this);
    ...
    public GreasedRegion2 surface8way()
    {
        GreasedRegion2 cpy = new GreasedRegion2(this).retract8way();
        return xor(cpy);
    }

    public GreasedRegion2 surface8way(int amount)
    {
        GreasedRegion2 cpy = new GreasedRegion2(this).retract8way(amount);
        return xor(cpy);
    }

    public GreasedRegion2[] surfaceSeries8way(int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        GreasedRegion2 temp = new GreasedRegion2(this);
    ...
    public ArrayList<GreasedRegion2> surfaceSeriesToLimit8way()
    {
        ArrayList<GreasedRegion2> regions = retractSeriesToLimit8way();
        if(regions.isEmpty())
            return regions;
        regions.add(0, regions.get(0).copy().xor(this));
        for (int i = 1; i < regions.size() - 1; i++) {
    ...
    public GreasedRegion2 flood(GreasedRegion2 bounds)
    {
    ...
    public GreasedRegion2 flood(GreasedRegion2 bounds, int amount)
    {
        int ct = size(), ct2;
        for (int i = 0; i < amount; i++) {
            flood(bounds);
    ...
    public GreasedRegion2[] floodSeries(GreasedRegion2 bounds, int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        int ct = size(), ct2;
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        ...
        for (int i = 0; i < amount; i++) {
            if(done) {
                regions[i] = new GreasedRegion2(temp);
            }
            else {
                regions[i] = new GreasedRegion2(temp.flood(bounds));
                if (ct == (ct2 = temp.size()))
                    done = true;
                else
                    ct = ct2;
    ...
    public ArrayList<GreasedRegion2> floodSeriesToLimit(GreasedRegion2 bounds) {
        int ct = size(), ct2;
        ArrayList<GreasedRegion2> regions = new ArrayList<>();
        GreasedRegion2 temp = new GreasedRegion2(this);
        while (true) {
    ...
    public GreasedRegion2 flood8way(GreasedRegion2 bounds)
    {
        if(width < 2 || ySections <= 0 || bounds == null || bounds.width < 2 || bounds.ySections <= 0)
            return this;
    ...
    public GreasedRegion2 flood8way(GreasedRegion2 bounds, int amount)
    {
        int ct = size(), ct2;
        for (int i = 0; i < amount; i++) {
            flood8way(bounds);
    ...
    public GreasedRegion2[] floodSeries8way(GreasedRegion2 bounds, int amount)
    {
        if(amount <= 0) return new GreasedRegion2[0];
        int ct = size(), ct2;
        GreasedRegion2[] regions = new GreasedRegion2[amount];
        ...
        for (int i = 0; i < amount; i++) {
            if(done) {
                ...
                regions[i] = new GreasedRegion2(temp.flood8way(bounds));
                if (ct == (ct2 = temp.size()))
                    done = true;
                else
                    ct = ct2;
    ...
    public ArrayList<GreasedRegion2> floodSeriesToLimit8way(GreasedRegion2 bounds) {
        int ct = size(), ct2;
        ArrayList<GreasedRegion2> regions = new ArrayList<>();
        GreasedRegion2 temp = new GreasedRegion2(this);
        while (true) {
    ...
    public GreasedRegion2 spill(GreasedRegion2 bounds, int volume, IRNG rng)
    {
        if(width < 2 || ySections <= 0 || bounds == null || bounds.width < 2 || bounds.ySections <= 0)
            return this;
        int current = size();
        if(current >= volume)
            return this;
        GreasedRegion2 t = new GreasedRegion2(this).notAnd(bounds);
...
//                b2[p] &= ~(1L << (y & 63));
    ...
    public GreasedRegion2 removeCorners()
    {
        if(width <= 2 || ySections <= 0)
            return this;
    ...
    /**
     * If this GreasedRegion2 stores multiple unconnected "on" areas, this finds each isolated area (areas that
     * are only adjacent diagonally are considered separate from each other) and returns it as an element in an
     * ArrayList of GreasedRegion2, with one GreasedRegion2 per isolated area. Not to be confused with
     ...
     * <br>
     * {@code GreasedRegion2 floors = new GreasedRegion2(map, '.'),
     * rooms = floors.copy().retract8way().flood(floors, 2),
     * corridors = floors.copy().andNot(rooms),
     * doors = rooms.copy().and(corridors.copy().fringe());}
     * <br>
     * You can then get all rooms as separate regions with {@code List<GreasedRegion2> apart = split(rooms);}, or
     ...
     * class.
     * @see squidpony.squidgrid.mapping.RoomFinder for a class that uses this technique without exposing GreasedRegion2
     * @return an ArrayList containing each unconnected area from packed as a GreasedRegion2 element
     */
    public ArrayList<GreasedRegion2> split()
    {
        ArrayList<GreasedRegion2> scattered = new ArrayList<>(32);
        int fst = firstTight();
        GreasedRegion2 remaining = new GreasedRegion2(this);
        while (fst >= 0) {
            GreasedRegion2 filled = new GreasedRegion2(width, height).insert(fst).flood(remaining, width * height);
     ...
     * ArrayList of GreasedRegion2, with one GreasedRegion2 per isolated area. This should not be confused with
     ...
     * @return an ArrayList containing each unconnected area from packed as a GreasedRegion2 element
     */
    public ArrayList<GreasedRegion2> split8way()
    {
        ArrayList<GreasedRegion2> scattered = new ArrayList<>(32);
        int fst = firstTight();
        GreasedRegion2 remaining = new GreasedRegion2(this);
        while (fst >= 0) {
            GreasedRegion2 filled = new GreasedRegion2(width, height).insert(fst).flood8way(remaining, width * height);
            scattered.add(filled);
            remaining.andNot(filled);
            fst = remaining.firstTight();
        }
    ...
    /**
     * Finds the largest contiguous area of "on" cells in this GreasedRegion2 and returns it; does not modify this
     * GreasedRegion2. If there are multiple areas that are all equally large with no larger area, this returns the
     * region it checks first and still is largest (first determined by the same ordering {@link #nth(int)} takes).
     * This may return an empty GreasedRegion2 if there are no "on" cells, but it will never return null.
     ...
     * @return a new GreasedRegion2 that corresponds to the largest contiguous sub-region of "on" cells in this.
     */
    public GreasedRegion2 largestPart()
    {
        int fst = firstTight(), bestSize = 0, currentSize;
        GreasedRegion2 remaining = new GreasedRegion2(this), filled = new GreasedRegion2(width, height),
                choice = new GreasedRegion2(width, height);
        while (fst >= 0) {
     ...
     * @return a new GreasedRegion2 that corresponds to the largest contiguous sub-region of "on" cells in this.
     */
    public GreasedRegion2 largestPart8way()
    {
        int fst = firstTight(), bestSize = 0, currentSize;
        GreasedRegion2 remaining = new GreasedRegion2(this), filled = new GreasedRegion2(width, height),
                choice = new GreasedRegion2(width, height);
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor upwards when this is called.
     ...
     */
    public GreasedRegion2 neighborUp()
    {
        if(width < 2 || ySections <= 0)
            return this;
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor downwards when this is called.
     * Down is defined as positive y. Neighbors are "on" cells exactly one cell away. A cell can have a neighbor
     * without itself being on; this is useful when finding the "shadow" cast away from "on" cells in one direction.
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborDown()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = 0; a < ySections; a++) {
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor to the left when this is called.
     * Left is defined as negative x. Neighbors are "on" cells exactly one cell away. A cell can have a neighbor
     * without itself being on; this is useful when finding the "shadow" cast away from "on" cells in one direction.
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborLeft()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = 0; a < ySections; a++) {
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor to the right when this is called.
     * Right is defined as positive x. Neighbors are "on" cells exactly one cell away. A cell can have a neighbor
     * without itself being on; this is useful when finding the "shadow" cast away from "on" cells in one direction.
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborRight()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = 0; a < ySections; a++) {
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor upwards and to the left when this
     ...
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborUpLeft()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = ySections - 1; a >= 0; a--) {
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor upwards and to the right when
     ...
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborUpRight()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = ySections - 1; a >= 0; a--) {
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor downwards and to the left when
     ...
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborDownLeft()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = 0; a < ySections; a++) {
    ...
    /**
     * Modifies this GreasedRegion2 so the only cells that will be "on" have a neighbor downwards and to the right when
     * this is called. Down is defined as positive y, right as positive x. Neighbors are "on" cells exactly one cell
     * away. A cell can have a neighbor without itself being on; this is useful when finding the "shadow" cast away from
     * "on" cells in one direction.
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 neighborDownRight()
    {
        if(width < 2 || ySections <= 0)
            return this;
        for (int a = 0; a < ySections; a++) {
    ...
    public GreasedRegion2 removeIsolated()
    {
        int fst = firstTight();
        GreasedRegion2 remaining = new GreasedRegion2(this), filled = new GreasedRegion2(this);
        while (fst >= 0) {
    ...
    /**
     * Returns true if any cell is "on" in both this GreasedRegion2 and in other; returns false otherwise. For example,
     * if (1,1) is "on" in this and (1,1) is "on" in other, this would return true, regardless of other cells.
     * @param other another GreasedRegion2; its size does not have to match this GreasedRegion2's size
     * @return true if this shares any "on" cells with other
     */
    public boolean intersects(GreasedRegion2 other)
    {
        if(other == null)
            return false;
        for (int x = 0; x < width && x < other.width; x++) {
    ...
    public static OrderedSet<GreasedRegion2> whichContain(int x, int y, GreasedRegion2... packed)
    {
        OrderedSet<GreasedRegion2> found = new OrderedSet<>(packed.length);
        GreasedRegion2 tmp;
    ...
    public static OrderedSet<GreasedRegion2> whichContain(int x, int y, Collection<GreasedRegion2> packed)
    {
        OrderedSet<GreasedRegion2> found = new OrderedSet<>(packed.size());
        for (GreasedRegion2 tmp : packed) {
    ...
    /**
     * Gets a Coord array from the "on" contents of this GreasedRegion2, using a deterministic but random-seeming
    ...
    /**
     * Modifies this GreasedRegion2 so it contains a deterministic but random-seeming subset of its previous contents,
     ...
     * @return this for chaining
     */
    public GreasedRegion2 mixedRandomRegion(double fraction) {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 mixedRandomRegion(double fraction, int limit) {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 mixedRandomRegion(double fraction, int limit, long seed) {
        int idx, run = 0;
        final int total = size();
        if (total <= limit)
            return this;
    ...
    /**
     * Gets a Coord array from the "on" contents of this GreasedRegion2, using a quasi-random scattering of chosen cells
    ...
    /**
     * Modifies this GreasedRegion2 so it contains a quasi-random subset of its previous contents, choosing cells so that
     ...
     * @return this for chaining
     */
    public GreasedRegion2 quasiRandomRegion(double fraction) {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 quasiRandomRegion(double fraction, int limit) {
     ...
     * running {@link #surface()} on a copy of this GreasedRegion2, running {@link #quasiRandomRegion(double)} on that
     * surface with the given fractionKept, taking the original GreasedRegion2 and removing its whole surface with
     * {@link #retract()}, then inserting the quasi-randomly-removed surface into this GreasedRegion2 to replace its
     ...
     * @return this for chaining
     */
    public GreasedRegion2 fray(double fractionKept)
    {
        GreasedRegion2 cpy = new GreasedRegion2(this).retract();
    ...
    /**
     * Modifies this GreasedRegion2 so it contains a random subset of its previous contents, choosing cells so that the
     ...
     * @return this for chaining
     */
    public GreasedRegion2 randomScatter(IRNG rng, int minimumDistance) {
     ...
     * @return this for chaining
     */
    public GreasedRegion2 randomScatter(IRNG rng, int minimumDistance, int limit) {
        int tmp, total = 0, ct;
        tally();
        if(this.ct == 0)
            return this;
    ...
    {
        GreasedRegion2 me2 = copy().surface8way();
    ...
    /**
     * Calculates a perceptual hash for this GreasedRegion2 using a method that is only precise for some sizes of
     * GreasedRegion2; it writes a result to into, and uses working as a temporary buffer. The lengths of into and
    ...
    */
    ...
    /**
     * Gets a single random Coord from the "on" positions in this GreasedRegion2, or the Coord (-1,-1) if this is empty.
     ...
     * faster than the previous method. The fastest way to get a random Coord from a GreasedRegion2 is to avoid iterating
     ...
     * @return a single randomly-chosen Coord from the "on" positions in this GreasedRegion2, or (-1,-1) if empty
     */
    public Coord singleRandom(IRNG rng)
    {
        int ct = size(), tmp = rng.nextInt(ct);
        Gdx.app.log("count", StringKit.join(", ", counts));
        Gdx.app.log("random bounded", Integer.toString(tmp));
        long t, w;
        for (int s = 0; s < ySections; s++) {
            for (int x = 0; x < width; x++) {
                if ((ct = counts[x * ySections + s]) > tmp) {
     ...
     * points in a GreasedRegion2 less predictably.
    ...
    /**
     * Like {@link #nth(int)}, this gets the Coord at a given index along a path through the GreasedRegion2, but unlike
     ...
     * @param index the distance along the Z-order curve to travel, only counting "on" cells in this GreasedRegion2.
    ...
    /**
     * Like {@link #nth(int)}, this finds a given index along a path through the GreasedRegion2, but unlike nth(), this
     ...
     * cells in the GreasedRegion2 eventually, and the tight encoding may be handy if you need to use ints.
     * @param index the distance along the Z-order curve to travel, only counting "on" cells in this GreasedRegion2.
     ...
     * outside the GreasedRegion2 but are on the Z-Order Curve.
     * Does not restrict the size of the returned array other than only using up to {@code fraction * size()} cells.
     * @param fraction the fraction of "on" cells to randomly select, between 0.0 and 1.0
     * @return a freshly-allocated Coord array containing the quasi-random cells
     */
     ...
     * are outside the GreasedRegion2 but are on the Z-Order Curve.
     * Restricts the total size of the returned array to a maximum of {@code limit} (minimum is 0 if no cells are "on").
     * If limit is negative, this will not restrict the size.
     * @param fraction the fraction of "on" cells to randomly select, between 0.0 and 1.0
     * @param limit the maximum size of the array to return
     ...
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 separatedRegionZCurve(double fraction)
    {
     ...
     * @return this, after modifications, for chaining
     */
    public GreasedRegion2 separatedRegionZCurve(double fraction, int limit)
    {
        if(fraction <= 0)
            return empty();
        if(fraction >= 1)
    ...
    public GreasedRegion2 randomRegion(IRNG rng, int size)
    {
        int ct = size(), idx = 0, run = 0;
        if(ct <= 0 || size <= 0)
            return empty();
     ...
     * a position once for every GreasedRegion2 that has that cell as "on." This means if you give 8 GreasedRegions to
     ...
     */
    public static int[][] sum(GreasedRegion2... regions)
    {
     ...
     * @return a 2D int array with the same width and height as the regions, where an int cell equals the number of given GreasedRegions that had an "on" cell at that position
     */
    public static int[][] sum(List<GreasedRegion2> regions)
    {
        if(regions == null || regions.isEmpty())
            return new int[0][0];
        GreasedRegion2 t = regions.get(0);
     ...
     */
    public static double[][] sumDouble(GreasedRegion2... regions)
    {
        if(regions == null || regions.length <= 0)
            return new double[0][0];
        int w = regions[0].width, h = regions[0].height, l = regions.length, ys = regions[0].ySections;
     ...
     * @return a 2D double array with the same width and height as the regions, where an double cell equals the number of given GreasedRegions that had an "on" cell at that position
     */
    public static double[][] sumDouble(List<GreasedRegion2> regions)
    {
        if(regions == null || regions.isEmpty())
            return new double[0][0];
        GreasedRegion2 t = regions.get(0);
     ...
     * and, for every GreasedRegion2 that has that cell as "on," adding the int in the corresponding weights array at
     ...
     */
    public static int[][] sumWeighted(GreasedRegion2[] regions, int[] weights)
    {
     ...
     * all 0 and, for every GreasedRegion2 that has that cell as "on," adding the double in the corresponding weights
     ...
     */
    public static double[][] sumWeightedDouble(GreasedRegion2[] regions, double[] weights)
    {
        if(regions == null || regions.length <= 0 || weights == null || weights.length < regions.length)
            return new double[0][0];
        int w = regions[0].width, h = regions[0].height, l = regions.length, ys = regions[0].ySections;
     ...
     */
    public static int[][] sumInto(int[][] existing, GreasedRegion2... regions)
    {
        ...
        for (int i = 0; i < l; i++) {
            GreasedRegion2 region = regions[i];
            ys = region.ySections;
     ...
     * at a position once for every GreasedRegion2 that has that cell as "on." This means if you give 8 GreasedRegions to
     * this method, it can increment by any number between 0 and 8 in a cell; if you give 16 GreasedRegions, then it can
     * increase the value in existing by any number between 0 and 16 in a cell.
     ...
     */
    public static double[][] sumIntoDouble(double[][] existing, GreasedRegion2... regions)
    {
        if(regions == null || regions.length <= 0 || existing == null || existing.length == 0 || existing[0].length == 0)
            return existing;
    ...
    public static double[][] dijkstraScan(char[][] map, Coord... goals)
    {
        ...
        double[][] numbers = new double[w][h];
        GreasedRegion2 walls = new GreasedRegion2(map, '#'), floors = new GreasedRegion2(walls).not(),
                middle = new GreasedRegion2(w, h, goals).and(floors);
        ArrayList<GreasedRegion2> regions = middle.floodSeriesToLimit(floors);
        int l = regions.size();
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                for (int i = 0; i < l; i++) {
    ...
    public static double[][] dijkstraScan8way(char[][] map, Coord... goals)
    {
        ...
        GreasedRegion2 walls = new GreasedRegion2(map, '#'), floors = new GreasedRegion2(walls).not(),
                middle = new GreasedRegion2(w, h, goals).and(floors);
        ArrayList<GreasedRegion2> regions = middle.floodSeriesToLimit8way(floors);
        int l = regions.size();
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                for (int i = 0; i < l; i++) {
     ...
     */
    public static int[][] bitSum(GreasedRegion2... regions)
    {
        if(regions == null || regions.length <= 0)
            return new int[0][0];
    ...
    /*
    public static int[][] selectiveNegate(int[][] numbers, GreasedRegion2 region, int mask)
    {
        if(region == null)
            return numbers;
    ...
    */
    ...
    public boolean equals(Object o) {
        ...
        GreasedRegion2 that = (GreasedRegion2) o;
    ...
    public static GreasedRegion2 deserializeFromString(String s)
    {
        if(s == null || s.isEmpty())
            return null;
        ...
        return new GreasedRegion2(data, w, h);
    }

    /**
     * Constructs a GreasedRegion2 using a vararg for data. Primarily meant for generated code, since
     * {@link #serializeToString()} produces a String that happens to be a valid parameter list for this method.
     * @param width width of the GreasedRegion2 to produce
     * @param height height of the GreasedRegion2 to produce
     * @param data array or vararg of long containing the exact data, probably from an existing GreasedRegion2
     * @return a new GreasedRegion2 with the given width, height, and data
     */
    public static GreasedRegion2 of(final int width, final int height, final long... data)
    {
        return new GreasedRegion2(data, width, height);
    }
    ...
    public boolean retainAll(Collection<?> c) {
        GreasedRegion2 g2 = new GreasedRegion2(width, height);
        for(Object o : c)
        {
    ...
    /**
     * Randomly removes points from a GreasedRegion2, with larger values for preservation keeping more of the existing
     ...
     * @return a randomly modified change to this GreasedRegion2
     */
    public GreasedRegion2 deteriorate(RandomnessSource rng, int preservation) {
    ...
    /**
     * Randomly removes points from a GreasedRegion2, with preservation as a fraction between 1.0 (keep all) and 0.0
     ...
     * points without change (returning this GreasedRegion2), while anything less than 0.015625 (1.0/64) will empty this
     * GreasedRegion2 (using {@link #empty()}) and then return it. The parameter {@code random} can be an object like a
     ...
     * @return a randomly modified change to this GreasedRegion2
     */
    public GreasedRegion2 deteriorate(final RandomnessSource random, final double preservation) {
     ...
     */
    public GreasedRegion2 flip(int x, int y) {
    ...
    /**
     * Returns a new GreasedRegion2 that has been mirrored along the rightmost edge, parallel to the y-axis. The new
     * GreasedRegion2 will have exactly twice the width, the additional width will have the contents of the original
     ...
     * @return a new GreasedRegion2 with twice the width of {@code this}, that is mirrored along the rightmost edge
     */
    public GreasedRegion2 mirrorY()
    {
        GreasedRegion2 next = new GreasedRegion2(data, width, height, width * 2, height);
    ...
    public boolean intersectsWith(Zone other) {
        if (other instanceof GreasedRegion2)
            return intersects((GreasedRegion2) other);
        long t, w;
        for (int x = 0; x < width; x++) {
            for (int s = 0; s < ySections; s++) {
                if ((t = data[x * ySections + s]) != 0) {
     ...
     */
    @Override
    public GreasedRegion2 translate(Coord c) {
        return copy().translate(c.x, c.y);
    }
    /**
     * Gets a Collection of Coord values that are not in this GreasedRegion2, but are
     * adjacent to it, either orthogonally or diagonally. Related to the fringe()
     * methods in CoordPacker and GreasedRegion2, but guaranteed to use 8-way
     * adjacency and to return a new Collection of Coord. This implementation returns
     * a GreasedRegion2 produced simply by {@code return copy().fringe8way();} .
     * @return Cells adjacent to {@code this} (orthogonally or diagonally) that
     * aren't in {@code this}
     */
    @Override
    public GreasedRegion2 getExternalBorder() {
     ...
     * CoordPacker and GreasedRegion2, but guaranteed to use 8-way adjacency
     * and to return a new Zone. This implementation returns a GreasedRegion2
     * produced simply by {@code return copy().expand8way();} .
     * @return A new GreasedRegion2 where "off" cells adjacent to {@code this}
     *         (orthogonally or diagonally) have been added to the "on" cells
     *         in {@code this}
     */
    @Override
    public GreasedRegion2 extend() {
        return copy().expand8way();
    }

    /**
     * Checks if {@code c} is present in this GreasedRegion2. Returns true if and only if c is present in this
     * GreasedRegion2 as an "on" cell. This will never be true if c is null, has negative x or y, has a value for x that
     ...
     * @param c a Coord to try to find in this GreasedRegion2; if null this will always return false
     * @return true if {@code c} is an "on" cell in this GreasedRegion2, or false otherwise, including if c is null
     */
    @Override
     ...
     * @param other another Zone, such as a GreasedRegion2 or a {@link squidpony.squidgrid.zone.CoordPackerZone}
     * @return true if all Coords in other are "on" in this GreasedRegion2, or false otherwise
     */
    @Override
    ...
    /**
     * Gets the distance between the minimum x-value contained in this GreasedRegion2 and the maximum x-value in it.
     * Not the same as accessing the field {@link #width} on a GreasedRegion2! The field will get the span of the space
     * that the GreasedRegion2 can use, including "on" and "off" cells. This method will only get the distance between
     * the furthest-separated "on" cells on the x-axis, and won't consider "off" cells. This method can return -1 if the
     * GreasedRegion2 is empty, 0 if the "on" cells are all in a vertical line (that is, when the minimum x is equal to
    ...
    /**
     * Gets the distance between the minimum y-value contained in this GreasedRegion2 and the maximum y-value in it.
     * Not the same as accessing the field {@link #height} on a GreasedRegion2! The field will get the span of the space
     * that the GreasedRegion2 can use, including "on" and "off" cells. This method will only get the distance between
     * the furthest-separated "on" cells on the y-axis, and won't consider "off" cells. This method can return -1 if the
     * GreasedRegion2 is empty, 0 if the "on" cells are all in a horizontal line (that is, when the minimum y is equal to
    ...
    /**
     * Gets the diagonal distance from the point combining the lowest x-value present in this GreasedRegion2 with the
...
//    }
    ...
    @Override
    public GreasedRegion2 getInternalBorder() {
