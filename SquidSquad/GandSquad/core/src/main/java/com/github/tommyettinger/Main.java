...
//        long seed = TimeUtils.millis() >>> 21; // use to keep each seed for about half an hour; useful for testing
        ...
        input.setInputProcessor(new InputAdapter(){
            ...
            // receive highlighting). Uses GradientGrid.findPathPreScanned() to find the path, which is rather fast.
            @Override
            public boolean mouseMoved(int screenX, int screenY) {
                ...
                if (onGrid(screenX = MathUtils.floor(pos.x), screenY = MathUtils.floor(pos.y))) {
                    ...
                    // This uses GradientGrid.findPathPreScannned() to get a path as a List of Coord from the current
                    // player position to the position the user clicked on. The "PreScanned" part is an optimization
                    // that's special to GradientGrid; because the part of the map that is viable to move into has
                    // already been fully analyzed by the GradientGrid.partialScan() method at the start of the
...
//        FOV.reuseFOV(res, light, playerX, playerY, LineWobble.wobble(12345, modifiedTime) * 2.5f + 4f, Radius.CIRCLE);
    ...
    public void render() {
        ...
        {
            PointI2 m = awaitedMoves.removeFirst();
            if (!toCursor.isEmpty())
                toCursor.removeFirst();
            move(Direction.ALL[com.github.tommyettinger.gand.utils.Direction.toGoTo(playerGlyph.getLocation(), (m)).ordinal()]);
        }
        else {
...
//                postMove();
                ...
                if (awaitedMoves.isEmpty()) {
                    ...
                    // player's position, and the "target" of a pathfinding method like GradientGrid.findPathPreScanned() is the
                    // currently-moused-over cell, which we only need to set where the mouse is being handled.
                    playerToCursor.setGoal(playerGlyph.getLocation());
                    // GradientGrid.partialScan only finds the distance to get to a cell if that distance is less than some limit,
