...
import static io.anuke.arc.input.KeyCode.Q;

/**
 */
public class GraphicalDemo extends ApplicationCore {
    ...
    public void init () {
        ...
        font = new BitmapFont(Core.files.internal("NanoOKExtended.fnt"), atlas.find("NanoOKExtended"));
        ...
        charMapping.put('.', atlas.find("Feature_Pebbles"    ));
        charMapping.put(',', atlas.find("Feature_Waves"      ));
        charMapping.put('~', atlas.find("Feature_Waves"      ));
        charMapping.put('"', atlas.find("Feature_Grass"      ));
        charMapping.put('#', atlas.find("Feature_Pillar"     ));
        charMapping.put('+', atlas.find("Feature_Door_Closed"));
        charMapping.put('/', atlas.find("Feature_Door_Open"  ));
        charMapping.put('└', atlas.find("Wall_ES"            ));
        charMapping.put('┌', atlas.find("Wall_NE"            ));
        charMapping.put('┬', atlas.find("Wall_NEW"           ));
        charMapping.put('┴', atlas.find("Wall_ESW"           ));
        charMapping.put('─', atlas.find("Wall_EW"            ));
        charMapping.put('│', atlas.find("Wall_NS"            ));
        charMapping.put('├', atlas.find("Wall_NES"           ));
        charMapping.put('┼', atlas.find("Wall_NESW"          ));
        charMapping.put('┤', atlas.find("Wall_NSW"           ));
        charMapping.put('┐', atlas.find("Wall_NW"            ));
        charMapping.put('┘', atlas.find("Wall_SW"            ));
        ...
        playerSprite = new Sprite(atlas.find(rng.getRandomElement(possibleCharacters)));
        ...
        for (int i = 0; i < numMonsters; i++) {
            ...
            Sprite monster = new Sprite(atlas.find(rng.getRandomElement(possibleEnemies)));
        ...
        input = new InputProcessor() {
            ...
            public boolean touchUp(int screenX, int screenY, int pointer, KeyCode button) {
                ...
                if (onGrid(Mathf.floor(pos.x) >> 4, Mathf.floor(pos.y) >> 4)) {
            ...
            public boolean mouseMoved(int screenX, int screenY) {
                ...
                pos.set(screenX, screenY);
                mainViewport.unproject(pos);
                if (onGrid(screenX = Mathf.floor(pos.x) >> 4, screenY = Mathf.floor(pos.y) >> 4)) {
    ...
    public void putMap()
    {
        ...
        for (int i = 0; i < bigWidth; i++) {
            for (int j = 0; j < bigHeight; j++) {
                if(visible[i][j] > 0.0) {
                    pos.set(i * cellWidth, j * cellHeight);
                    Draw.color(toCursor.contains(Coord.get(i, j))
                    ...
                    Draw.rect(solid, pos.x, pos.y);
                    if((monster = monsters.get(Coord.get(i, j))) != null)
                    {
                        monster.setAlpha(1f);
                        Draw.color(monster.getPackedColor());
                        monster.draw();
                    }
                    if(monster == null && visible[i][j] < 1.0)
                    {
                        Draw.color(ColorTools.lerpFloatColors(colors[i][j], FLOAT_LIGHTING, (float)visible[i][j] * 0.75f + 0.25f));
                        Draw.rect(charMapping.get(lineDungeon[i][j], solid), pos.x, pos.y);
                    }
                } else if(seen.contains(i, j)) {
                    pos.set(i * cellWidth, j * cellHeight);
                    Draw.color(ColorTools.lerpFloatColors(bgColors[i][j], FLOAT_GRAY, 0.7f));
                    Draw.rect(solid, pos.x, pos.y);
//                    if ((monster = monsters.get(Coord.get(i, j))) != null)
//                        monster.setAlpha(0f);
                    if(!monsters.containsKey(Coord.get(i, j)) && visible[i][j] < 1.0)
                    {
                        Draw.color(ColorTools.lerpFloatColors(colors[i][j], FLOAT_GRAY, 0.7f));
                        Draw.rect(charMapping.get(lineDungeon[i][j], solid), pos.x, pos.y);
                    }
                }
            }
...
//        }
        Draw.color(playerColor);
        playerSprite.draw();
        Draw.flush();
        Core.graphics.setTitle(Core.graphics.getFramesPerSecond() + " FPS");
    ...
    public void update () {
        ...
        if (health <= 0) {
            ...
            font.draw("YOUR CRAWL IS OVER!", x, y + 48, wide, Align.center, true);
            font.draw("A monster sniffs your corpse and says,", x, y + 16, wide, Align.center, true);
            font.draw(lang, x, y - 16, wide, Align.center, true);
            font.draw("q to quit.", x, y - 80, wide, Align.center, true);
            if(Core.input.keyDown(Q))
                Core.app.exit();
            return;
        }
