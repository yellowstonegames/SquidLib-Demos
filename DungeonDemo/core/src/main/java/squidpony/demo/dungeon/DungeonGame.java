    ...
    public void putMap()
    {
        ...
        for (int i = 0; i < gridWidth; i++) {
            for (int j = 0; j < gridHeight; j++) {
                if(fovmap[i][j] > 0)
                    alter = 0.15f + 0.55f * (float)fovmap[i][j] + 0.1f * FastNoise.instance.getConfiguredNoise(i, j, counter * 0.3f);
                else
                    alter = 0.1f;
                if(toCursor.contains(Coord.get(i, j)))
                    alter += 0.2f;
                display.putWithConsistentLight(i, j, lineDungeon[i][j],
                        SColor.LIMITED_PALETTE[colorIndices[i][j]],
                        SColor.LIMITED_PALETTE[bgColorIndices[i][j]],
                        SColor.PAPAYA_WHIP, alter);
            }
        }
    }
    @Override
